...
function CompanyPicker({ onSelect, placeholder }) {
  const { t } = useT()
  ...
  async function search(e) {
    ...
    try {
      const d = await api.post('/maps/search', { query: query.trim() })
      setResults(d.places || [])
  ...
  async function pick(place) {
    ...
    try {
      const d = await api.post('/maps/details', { place_id: place.place_id })
      onSelect(d)
    ...
    <div>
      <form onSubmit={search} className="flex gap-2">
        <input
          ...
          placeholder={placeholder || t('Firma adı ve şehir...')}
      ...
      {err && <div className="text-xs text-[#DC2626] mt-2">{err}</div>}
      {results.length > 0 && (
        <div className="mt-3 border border-[#E5E7EB] rounded-md divide-y divide-[#F3F4F6]">
          {results.map(r => (
            <button key={r.place_id} onClick={() => pick(r)} className="w-full text-left px-3 py-2 hover:bg-[#F9FAFB]">
      ...
      {loading && <div className="text-xs text-[#9CA3AF] mt-2">{t('Firma bilgileri alınıyor...')}</div>}
    </div>
  )
}

// Tool 1: find a company and generate a platform-specific outreach email.
function MailTool({ platform, platformName }) {
  const { t } = useT()
  ...
  useEffect(() => { setTarget(null); setEmail(null) }, [platform])
  ...
  return (
    <div>
      <div className="bg-white border border-[#E5E7EB] rounded-md p-4 mb-4">
        <label className="block text-xs font-medium text-[#374151] mb-2">{t('Hedef Firma')}</label>
        <CompanyPicker onSelect={t2 => { setTarget(t2); setEmail(null) }} />
        {target && (
          <div className="mt-3 flex items-center justify-between border-t border-[#F3F4F6] pt-3">
            <div>
              ...
              <div className="text-xs text-[#9CA3AF]">{target.address}</div>
            </div>
            <button onClick={compose} disabled={composing}
              ...
              {composing ? t('Oluşturuluyor...') : `${platformName} ${t('adına mail')}`}
            </button>
          </div>
        )}
      </div>

      {error && <div className="mb-4 text-sm text-[#991B1B] bg-[#FEE2E2] border border-[#FECACA] rounded-md px-3 py-2">{error}</div>}

      {email && (
        <div className="bg-white border border-[#E5E7EB] rounded-md p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="text-xs font-semibold text-[#374151]">{platformName} → {target?.name}</div>
...
// Tool 2: Google Maps review sentiment analysis (standalone).
function SentimentTool() {
  ...
  async function run() {
    ...
    try {
      const d = await api.post('/maps/sentiment', { reviews: target.reviews, company_name: target.name })
      setSentiment(d.result || '')
  ...
  return (
    <div>
      <div className="bg-white border border-[#E5E7EB] rounded-md p-4 mb-4">
        <label className="block text-xs font-medium text-[#374151] mb-2">{t('Firma (Google Maps)')}</label>
        <CompanyPicker onSelect={t2 => { setTarget(t2); setSentiment('') }} />
      </div>
          ...
          <div className="flex items-start justify-between">
            ...
            {target.reviews?.length > 0
              ? <button onClick={run} disabled={loading}
                  ...
                  {loading ? t('Analiz ediliyor...') : t('Sentiment Analizi')}
                </button>
              : <span className="text-xs text-[#9CA3AF]">{t('Yorum bulunamadı')}</span>}
          </div>
          {sentiment && (
...
export default function Ataol() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="text-xs text-[#6B7280] mb-5">{t('Platform seçin; ardından firma bulup platforma özel mail oluşturun ya da Google Maps yorum analizi yapın.')}</p>

      {/* Platform selector */}
      <div className="bg-white border border-[#E5E7EB] rounded-md p-4 mb-4">
        <label className="block text-xs font-medium text-[#374151] mb-1">{t('Platform / Hizmet')}</label>
        <select
          value={platform}
        ...
        <p className="text-[11px] text-[#9CA3AF] mt-1.5">{activePlatform.url}</p>
      </div>

      {/* Tabs: two separate tools */}
      <div className="flex items-center gap-1 mb-4">
        <button
          onClick={() => setTab('mail')}
          className={`text-xs font-medium px-3 py-1.5 rounded-md border ${tab === 'mail' ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'text-[#6B7280] border-[#E5E7EB] hover:bg-[#F9FAFB]'}`}
        >
          {t('Firma Bul + Mail Oluştur')}
        </button>
        <button
          onClick={() => setTab('sentiment')}
          className={`text-xs font-medium px-3 py-1.5 rounded-md border ${tab === 'sentiment' ? 'bg-[#2563EB] text-white border-[#2563EB]' : 'text-[#6B7280] border-[#E5E7EB] hover:bg-[#F9FAFB]'}`}
        >
          {t('Google Maps Sentiment')}
        </button>
      </div>

      {tab === 'mail'
        ? <MailTool platform={platform} platformName={activePlatform.name} />
        : <SentimentTool />}
    </div>
  )
}
